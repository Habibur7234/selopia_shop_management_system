...
//init User datatable and load data
    ...
    let user_table = $('#user_dataTable').DataTable({
        ajax: {
            url: 'https://nafisa.selopian.us/user',
            dataSrc: 'data',
        },
        rowId: 'id',
        dom: 'Blfrtip',
    ...
    $("#delete_user").click(function () {
        ...
        $(this).text('Deleting...');
        $.ajax({
            url: nafisa_domain + '/user/' + rowData._id,
            type: 'DELETE',
            success: function (data) {
...
//init supplier datatable and load data
    let supplier_table = $('#supplier_dataTable').DataTable({
        order: [[0, 'desc']],
        "columnDefs": [
            {'visible': false, 'targets': 0},
            {"width": "20%", "targets": 1},
...
//init Department datatable and load data
 $('#department_dataTable').DataTable({
    ...
    ajax: {
        url: 'https://shakila.selopian.us/department',
        dataSrc: 'data',
    },
    ...
    columns: [
        {data: '_id'},
        {data: 'name'},
        {data: 'user_profile_department_id'},
        {data: '',
            render: function () {
                return '<button id="update_shopBtn"  class="btn btn-primary" toggle="tooltip" title="Edit" type="button" data-bs-toggle="modal"   data-bs-target="#update_shop_modal" ><svg class="icon icon-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>  '
...
//init Parent--------------------------------------------
$.ajax({
    url: 'https://riyadshop.selopian.us/category/byparent/0',
    type: 'GET',
    success: function (data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.status.code === 1) {
                ...
                //Success Notification
                notyf.success({
                    message: 'New Category Added  <strong>Successfully !</strong>',
                    duration: 7000,
                    icon: false
                });
...
$("#delete_category").click(function () {
    ...
    $.ajax({
        url: riyad_domain + '/category/' + rowData.id,
        type: 'DELETE',
        dataType: "json",
        success: function (data) {
            ...
            if (data.status.code === 1){
                ...
                notyf.error({
                    message: "<strong>Warning !</strong> Can't Delete Root Category",
                    duration: 7000,
                    icon: false
                });
                ...
                notyf.error({
                    message: "<strong>Warning !</strong> Can't Delete  Category",
                    duration: 7000,
                    icon: false
                });
